/*
    Carte interactive des communes Cités Éducatives
    ...
    <div id="sidebar" class="leaflet-sidebar collapsed">
        ...
        <div class="leaflet-sidebar-content">
            ...
            <div class="leaflet-sidebar-pane" id="download">
                ...
                <p>
                    La liste des cités éducatives est disponible sur 
                    <a href='https://www.data.gouv.fr/fr/datasets/programme-cites-educatives/' target="_blank">data.gouv.fr</a>.
                </p>
                <h5 style="font-family:'Marianne-Extrabold'">
            ...
            <div class="leaflet-sidebar-pane" id="a-propos">
                ...
                <p>
                    ...
                    ANCT, programme Cités Éducatives
                </p>
                <p>
...
const LeafletMap = {
    ...
    methods: {
            ...
            for(let i=0;i<geomData.length;i++) {
                let marker = new L.GeoJSON(geomData[i], {
                    ...
                    pointToLayer: function (feature, latlng) {
                        ...
                        circleMarker.setStyle({fillColor:getColor("cite")});
        ...
        onClick(code) {
            ...
            content.cites = this.data.find(f => content.insee_com == f.insee_com).cites;
            ...
            circle.setStyle({fillColor:this.getColor("cite")});
            glow.setStyle({fillColor:this.getColor("cite")});

            this.sidebar.open("home");
        },
        stylishTooltip(marker) {
            return `<span style="background-color:${this.getColor("cite")}">${marker.lib_com}</span>`
        },
...
function getCiteFromRawJSON(rawCitiesData){

    allFormatedCitiesData = []

    rawCitiesData.forEach((city) => {

        countCite = Math.max(...Object.keys(city).map((prop) => prop.match(/^(.+)_cite(\d+)$/)? parseInt(prop.match(/^(.+)_cite(\d+)$/)[2]):0))
        formatedCityData = [...Array(countCite).fill(0).map(item => ({ qpv: []}))]

        counter = 0
        for (prop in city) {
            const cite_prop = prop.match(/^(.+)_cite(\d+)$/);
            const qpv_prop = prop.match(/^QPV(\d+)_cite(\d+)$/)

            if (cite_prop && city[prop] && qpv_prop) {
                formatedCityData[parseInt(cite_prop[2])-1]['qpv'] = [...formatedCityData[parseInt(cite_prop[2])-1]['qpv'], city[prop]]
            }
            if (cite_prop && city[prop] && !qpv_prop) {
                formatedCityData[parseInt(cite_prop[2])-1][cite_prop[1]] = city[prop]
            }
            if (cite_prop && city[prop] && counter < parseInt(cite_prop[2])) {
                counter = parseInt(cite_prop[2])
            }
        }
        allFormatedCitiesData.push(
            {
                insee_com: city['insee_com'].toString(),
                lib_com: city['lib_com'],
                cites: [...formatedCityData.slice(0, counter)]
            })
